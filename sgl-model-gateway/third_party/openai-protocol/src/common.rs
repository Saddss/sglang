...
pub trait GenerationRequest: Send + Sync {
    ...
    /// Truncation-turn flag (see `enable_kv_evict` in chat.rs). Default false.
...
/// Validates stop sequences (non-empty strings; no count cap in this fork)
...
pub fn validate_stop(stop: &StringOrArray) -> Result<(), validator::ValidationError> {
    match stop {
        ...
        StringOrArray::Array(arr) => {
            // NOTE (fork patch): upstream capped stop sequences at 4 (OpenAI spec).
            // Our engines (vLLM) accept more, and callers send template stop sets
            // larger than 4, so we drop the cap and only keep the non-empty check.
            for s in arr {
                if s.is_empty() {
                    return Err(validator::ValidationError::new(
                        "stop sequences cannot be empty",
